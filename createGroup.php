...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    $nom=$_POST['nom'];
    $etd=$_POST['etd'];
$query="select * from groupInfo where username='$username'";
$result=mysqli_query($con,$query);
$data=mysqli_fetch_array($result);
if (mysqli_num_rows($result)==0){
    $query= "INSERT INTO grouplist (gName,loi,nom,etd,minBudget,maxBudget) VALUES ('$gName','$loi','$nom','$etd','$minBudget','$maxBudget')";
    mysqli_query($con,$query);
    $query="select gCode from grouplist where gName='$gName'";
    $result=mysqli_query($con,$query);
    $data=mysqli_fetch_array($result);
    $gCode=$data[0];
    $query="INSERT INTO groupInfo (username,gCode) VALUES ('$username','$gCode')";
    mysqli_query($con,$query);
    $response="Group created Successfully!!";
    $hlocation='customerInterface.html';
    $ret="DashBoard";
}
else {
    $response="you are already in a Group!!";
    $hlocation='customerInterface.html';
    $ret="DashBoard";
}
}
    ...
    <header class="site-header">
      ...
      <div class='center'>
        <div class="container">
            ...
            <?php echo"<form action='$hlocation'>
            <input type='submit' name='submit' id='submit' class='submit' value='$ret'></form>";?>
        </div>
    </div>
